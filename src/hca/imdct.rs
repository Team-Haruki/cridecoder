...
use super::decoder::{HCA_MDCT_BITS, HCA_SAMPLES_PER_SUBFRAME, StChannel};
...
pub fn imdct_transform(ch: &mut StChannel, subframe: usize) {
            ...
            for _ in 0..count1 {
                for _ in 0..count2 {
                    let (a, b) = if use_temp_as_src {
                        ...
                        (ch.spectra[subframe][t1_idx], ch.spectra[subframe][t1_idx + 1])
                    };
                    t1_idx += 2;
    ...
    // Update output/IMDCT with overlapped window
    {
        ...
        for i in 0..half {
            ...
            ch.wave[subframe][i + half] = IMDCT_WINDOW[i + half] * dct[size - 1 - i] - ch.imdct_previous[i + half];
