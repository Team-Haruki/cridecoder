//! ATH (Audio Threshold Hearing) curve initialization

use super::decoder::HCA_SAMPLES_PER_SUBFRAME;

/// Base ATH curve data
const ATH_BASE_CURVE: [u8; 656] = [
    ,
];

/// Initialize ATH curve type 0 (all zeros)
pub fn ath_init_0(ath_curve: &mut [u8; HCA_SAMPLES_PER_SUBFRAME]) {
    ath_curve.fill(0);
}

/// Initialize ATH curve type 1 (sample-rate dependent)
pub fn ath_init_1(ath_curve: &mut [u8; HCA_SAMPLES_PER_SUBFRAME], sample_rate: u32) {
    let mut acc: u32 = 0;
    for i in 0..HCA_SAMPLES_PER_SUBFRAME {
        acc += sample_rate;
        let index = (acc >> 13) as usize;

        if index >= 654 {
            ath_curve[i..HCA_SAMPLES_PER_SUBFRAME].fill(0xFF);
            break;
        }
        ath_curve[i] = ATH_BASE_CURVE[index];
    }
}

/// Initialize ATH curve based on type
pub fn ath_init(
    ath_curve: &mut [u8; HCA_SAMPLES_PER_SUBFRAME],
    ath_type: u32,
    sample_rate: u32,
) -> bool {
    match ath_type {
        0 => {
            ath_init_0(ath_curve);
            true
        }
        1 => {
            ath_init_1(ath_curve, sample_rate);
            true
        }
        _ => false,
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_ath_init_0() {
        let mut curve = [0u8; HCA_SAMPLES_PER_SUBFRAME];
        ath_init_0(&mut curve);
        assert!(curve.iter().all(|&x| x == 0));
    }

    #[test]
    fn test_ath_init_1() {
        let mut curve = [0u8; HCA_SAMPLES_PER_SUBFRAME];
        ath_init_1(&mut curve, 44100);
        // With sample_rate=44100, acc starts at 44100, index = 44100 >> 13 = 5
        assert_eq!(curve[0], ATH_BASE_CURVE[5]);
        // Second iteration: acc=88200, index = 88200 >> 13 = 10
        assert_eq!(curve[1], ATH_BASE_CURVE[10]);
    }
}
